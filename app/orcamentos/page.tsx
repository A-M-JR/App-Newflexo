                ...
                <TableBody>
                  {loading && !dbData ? (
                    ...
                    <TableRow key={orc.id} onClick={() => router.push(`/orcamentos/${orc.id}`)} className="hover:bg-muted/30 transition-colors border-border/30 bg-card cursor-pointer">
                      ...
                      <TableCell className="max-w-[200px]"><div className="font-medium text-[13px] text-foreground truncate">{orc.cliente?.razaoSocial}</div>{orc.cliente?.nomeFantasia && <div className="text-[12px] text-muted-foreground truncate">{orc.cliente.nomeFantasia}</div>}{orc.cliente?.cnpj && <div className="text-[11px] text-muted-foreground/70 truncate font-mono font-normal">CNPJ: {orc.cliente.cnpj}</div>}</TableCell>
