                  ...
                  {loading && !dbData ? (
                    ...
                    return (
                      <TableRow key={ped.id} onClick={() => router.push(`/pedidos/${ped.id}`)} className={`hover:bg-muted/30 transition-colors border-border/30 bg-card cursor-pointer ${sla.class}`}>
                        ...
                        <TableCell><div className="font-medium text-[13px] text-foreground truncate">{ped.cliente?.razaoSocial}</div>{ped.cliente?.nomeFantasia && <div className="text-[12px] text-muted-foreground truncate">{ped.cliente.nomeFantasia}</div>}{ped.cliente?.cnpj && <div className="text-[11px] text-muted-foreground/70 truncate font-mono">CNPJ: {ped.cliente.cnpj}</div>}</TableCell>
