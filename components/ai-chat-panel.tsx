            ...
            if (name === 'gerar_orcamento') {
                ...
                if (filtrados.length === 0) {
                    ...
                    filtrados.forEach((c: any) => {
                        ...
                        const matrizes = c.etiquetasVinculadas || []
                        if (matrizes.length > 0) {
                            info += `🎨 **Matrizes:**\n${matrizes.map((m: any, idx: number) => `${idx + 1} - ${m.nome}`).join('\n')}\n`
                        }
                        info += `---\n`
                        resposta += info
                    })
                ...
                if (top.length === 0) {
                    ...
                    top.forEach(p => {
                        ...
                        resposta += `Status: ${p.statusObj?.nome}\n`
                ...
                const todas = await getEtiquetas()
                ...
                if (top.length === 0) {
                    ...
                    top.forEach((e, i) => {
                        resposta += `${i+1}. **${e.nome}**\nMaterial: ${e.material} | Medida: ${e.largura}x${e.altura}mm\n---\n`
                    })
                }
                addMessage({ role: "assistant", content: resposta })
            }
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-primary text-white">
                        <div className="flex items-center gap-2">
                            ...
                            <div className="leading-tight">
                                <h3 className="text-xs font-bold uppercase tracking-wider">Agente Newflexo</h3>
